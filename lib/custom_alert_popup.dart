...
///
/// [CustomAlertPopup] offers various features including support for custom widgets,
/// animations, countdown timers, and responsive design. This widget is highly
/// configurable to fit a wide range of use cases in your application.
class CustomAlertPopup extends StatelessWidget {
  ...
  const CustomAlertPopup({super.key, this.title, this.loading, this.imageWidget, this.buttonRadius, this.buttonSpacing, this.description, this.firstButtonTap, this.secondButtonTap, this.customWidget, this.firstButtonText, this.secondButtonText, this.width, this.height, this.imagePath, this.videoPath, this.imageHeight, this.timerDelay, this.hideButton, this.widget, this.mainAxisAlignment, this.crossAxisAlignment, this.descriptionAlign, this.titleSpacing, this.firstButtonWidth, this.secondButtonWidth, this.animationType = AnimationType.scale, this.animationDuration, this.fadeBegin = 0.0, this.hideCloseButton = true});
        ...
        children: [
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                if (imagePath != null) ...[Container(decoration: BoxDecoration(borderRadius: const BorderRadius.only(topLeft: Radius.circular(22), topRight: Radius.circular(22)), color: Colors.white, image: DecorationImage(image: AssetImage(imagePath ?? ''), fit: BoxFit.cover)), width: width ?? 430, height: imageHeight ?? 210)],
                ...
                Column(
                  ...
                  children: [
                    ...
                    if (description != null) ...[
                      ...
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 50), child: Text(description.toString(), style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w400, fontSize: 14, color: const Color(0xFF909090)), textAlign: TextAlign.center)),
                    ],
                      ...
                      child: Row(
                        children: [
                                      ...
                                      children: [
                                        ...
                                        if (snapshot.data != true)
                                          Positioned(
                                              ...
                                              bottom: 0,
                                              child: Container(
                                                  decoration: BoxDecoration(color: snapshot.data == true ? Colors.white : const Color(0xFFC4283C), borderRadius: BorderRadius.circular(14)),
                                                  width: isDesktop
                                                      ? width == null
                                                      ...
                                                      : 100,
                                                  height: 40,
                                                  child: Center(child: TimerCountdown(enableDescriptions: false, timeTextStyle: Theme.of(context).textTheme.titleSmall!.copyWith(color: Colors.white, fontWeight: FontWeight.bold), format: CountDownTimerFormat.secondsOnly, endTime: DateTime.now().add(Duration(seconds: timerDelay ?? 10))))))
                                      ],
                                    );
                                  }),
                            ),
                          if (timerDelay == null)
                            if (hideButton != true)
                              Expanded(
                                child: CustomButton(isLoading: loading ?? false, width: firstButtonWidth ?? (isDesktop ? 190 : 125), height: 45, text: firstButtonText ?? "Yes", color: const Color(0xFFC4283C), textColor: Colors.white, border: const Border(), borderRadius: BorderRadius.circular(buttonRadius ?? 14), onTap: () => {Navigator.of(context).pop(), firstButtonTap!()}),
                              ),
                          if (secondButtonText?.isNotEmpty == true) ...[
                            const SizedBox(width: 15),
                            Expanded(
                              child: CustomButton(isLoading: loading ?? false, width: secondButtonWidth ?? (isDesktop ? (hideButton != true ? 185 : 350) : (hideButton != true ? 100 : MediaQuery.of(context).size.width / 1.65)), height: 45, text: secondButtonText ?? "Cancel", color: const Color(0XFFFFF1F1), textColor: const Color(0xFFC4283C), border: const Border(), borderRadius: BorderRadius.circular(buttonRadius ?? 14), onTap: () => {Navigator.of(context).pop(), secondButtonTap!()}),
                            ),
                          ],
                        ],
                      ),
          ...
          if (hideCloseButton == true) Positioned(top: 16, right: 10, child: InkWell(borderRadius: BorderRadius.circular(100), onTap: () => Navigator.of(context).pop(), child: Container(height: 30, width: 30, decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0XFFFFF1F1)), child: const Icon(Icons.close, size: 20, color: Color(0xFFC4283C))))),
        ],
      ),
    );
